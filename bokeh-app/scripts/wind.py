...
def wind_tab(source):
    ...
    def make_speed_plot(source):
        p = figure(title='Wind Speed', x_axis_type='datetime', tools='crosshair,hover,pan,box_zoom,reset', plot_height=300, y_axis_label='Wind speed [m/s]')
    ...
    def make_dir_plot(source):
        p = figure(title='Wind Direction', x_axis_type='datetime', x_range=wind_plot.x_range, y_range=[-5, 365], tools='crosshair,hover,pan,box_zoom,reset', plot_height=300, y_axis_label='Wind direction [deg from N]')
    ...
    def make_ustar_plot(source):
        p = figure(title='Frictional Velocity', x_axis_type='datetime', x_range=wind_plot.x_range, tools='crosshair,hover,pan,box_zoom,reset', plot_height=300, y_axis_label='Frictional velocity (u*) [m/s]')
